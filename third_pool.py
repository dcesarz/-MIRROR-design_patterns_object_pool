...
class ThirdPool(metaclass=Singleton):
    ...
    def __init__(self, size):
        ...
        # [self._q.put(Third()) for _ in range(self._size)]
    ...
    def release(self, third):
        result = None
        while result is None:
            try:
                self._q.put(third, timeout=0.001)
                result = "Done!"
                self._q.maxsize -= 1
                self._counter_in_use -= 1
            except queue.Full:
...
class Third:
    ...
    def calculate(self):
        ...
        for i in self._num:
            temp = math.pow(
                (math.pow(math.fabs((self._weight_y / self._weight_x / self._weight_z)), math.fabs(1 / (i + 1)))),
                math.fabs(self._weight_x))
